...
import {
    Card, Typography, Box, Container, CardContent, Drawer, IconButton,
    Pagination, CircularProgress, Avatar, CardHeader, Chip
} from '@mui/material';
import { AddMessage, DeleteMessage, GetMessages } from './../../api/TeacherStore/TeacherStore';
...
function TeacherMessages() {
    ...
    const teacherMessages = useSelector((state) => state.teachers);
    ...
    const handleDeleteMessage = (id) => {
        dispatch(DeleteMessage(id)).catch((error) => {
            console.error("Error deleting message:", error);
        });
    };

    const sortedMessages = [...teacherMessages.messages].sort((a, b) => new Date(b.send_date) - new Date(a.send_date));
    ...
    const SendIcon = (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" className="bi bi-send-plus-fill" viewBox="0 0 16 16">
    ...
    return (
        <Container maxWidth="">
            ...
            <Box display="flex" flexDirection="row" alignItems="center" justifyContent="space-between">
                ...
                <Pagination count={Math.ceil(teacherMessages.messages.length / messagesPerPage)} page={page} onChange={handleChangePage} />
            </Box>
            {
                teacherMessages.status_message === 'loading' ?
                    <Box display="flex" justifyContent="center" alignContent={'center'} > <CircularProgress size={40}/> </Box> :
                    teacherMessages.status_message === 'failed' ? <Typography variant="body1" component="p" gutterBottom>{teacherMessages.error}</Typography> :
                        teacherMessages.messages.length === 0 ? <Typography variant="body1" component="p" gutterBottom>No messages found.</Typography> :
                            (
                                <Box mt={4} p={2}>
