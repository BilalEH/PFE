...
import { deleteParent } from "../../../../api/adminsStore/adminStore";
import { useDispatch } from "react-redux";

export default function DeleteParentPopup({open, setOpen, parent}) {
    const dispatch = useDispatch();

    const handleDelete = () => {
        dispatch(deleteParent(parent.id));
        setOpen(false);
    };
        ...
        <>
            {parent && (
                <Dialog open={open} onClose={() => setOpen(false)}>
                    <DialogTitle>
                        <div className="popup-title text-center">Delete Parent</div>
                    </DialogTitle>
                    <DialogContent>
                        <div className="popup-content">
                            <div className="popup-text my-3 text-center">
                                Are you sure you want to delete parent {" "}
                                <span className="popup-name">
                                    {parent.user_id.firstName} {parent.user_id.lastName}
                                </span>
                            </div>
